...
public class DataBuilder
{
    ...
    public IDataStore BuildData()
    {
        ...
        var serverLookup = _servers.Select(x => x.UniqueId).ToHashSet();
        var functionLookup = _functions.Select(x => x.Id).ToHashSet();
        
        foreach (var tip in _tips)
        {
            if (!serverLookup.Contains(tip.Recipient.UniqueId))
            {
        ...
        return new DataStore(_payPeriod, _servers, _functions, _tipTypes, _tips);
    }
